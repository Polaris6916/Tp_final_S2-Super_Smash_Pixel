    ...
    , m_timeText(nullptr)
{
    ...
    UIGridLayout *hLayout = new UIGridLayout(m_scene, 2, playerCount);
    ...
    for (int i = 0; i < playerCount; i++)
    {
        ...
        font = assets->GetFont(FONT_DAMAGE);
        text = new UIText(scene, "0%", font, Colors::White);
        text->SetAnchor(Anchor::CENTER);

        hLayout->AddObject(text, 1, i);

        m_koCountTexts.push_back(text);
    }
...
void UIStageHUD::Update()
{
    ...
    for (int i = 0; i < playerCount; i++)
    {
        ...
        const int score_kocount = static_cast<int>(g_gameCommon.playerStats[i].koCount);
        m_koCountTexts[i]->SetString(std::to_string(score_kocount) + " Ko count");
    }
}
