...
void FireKnightSystem::OnFixedUpdate(EntityCommandBuffer &ecb)
{
    ...
    for (auto [entity, affiliation, controller, anim] : viewAnim.each())
    {
        ...
        // TODO 9 - Commencez une animation du type RUN quand l'état du personnage
        //        passe à l'état PlayerState::RUN.

        // TODO 11 - Plus loin dans le tuto, pour la gestion des attaques,
        //        liez l'animation ATTACK_1 à l'état ATTACK_COMBO.

        // TODO 17 - Vraiement plus loin dans le tuto, pour le smash,
...
)
{
    ...
    if (animType ==  AnimType::ATTACK_1)
    {
        ...
        {
            // TODO 14 - Effectuez l'attaque sur la frame d'indice 4 et non celle d'indice 2.
            ...
            // TODO 14 - Modifiez le rayon du cercle d'attaque avec la valeur trouvée
            //        en utilisant les outils de debug dans le jeu.

            const b2Vec2 center = transform.position + b2Vec2{ s * 0.50f, 1.20f };
    ...
    {
        // TODO 13 - Modifier la vitesse automatique pour lui donner une valeur de 2 unité par seconde.
        //      - Utilisez la constante s pour modifier la direction.

        if (animEvent.index == 0)
        {
            animInfo.autoVelocity = 10.0f * s;
        }
    ...
    else if (animType == AnimType::ATTACK_3)
    {
        ...
        {
            // TODO 15 - Modifiez les dommages pour effectuer une ejection.
    ...
    else if (animType == AnimType::ATTACK_AIR)
    {
        ...
        {
            const b2Vec2 vertices[5] = {
                position + b2Vec2(s * 0.9f, 2.1f),
                position + b2Vec2(s * 3.6f, 2.1f),
                position + b2Vec2(s * 4.2f, 1.7f),
                position + b2Vec2(s * 3.f, 1.4f),
                position + b2Vec2(s * 0.5f, 1.5f),
            };
            bool hit = DamageUtils::AttackPolygon(m_scene, entity, affiliation, damage, filter, vertices, 5);
            PlayerUtils::PlaySFXHit(m_scene, playerID, SFX_SWORD_HIT_B1, SFXIntensity::WEAK, hit);
        }
    ...
    PlayerAnimInfo &event)
{
    ...
    {
        // TODO 12 - Décommenter l'enchaînement d'animation après la première attaque.
    ...
    // TODO 17 - Décommentez le code suivant.
