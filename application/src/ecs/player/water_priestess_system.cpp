...
void WaterPriestessSystem::OnUpdate(EntityCommandBuffer &ecb)
{
    ...
    for (auto [entity, affiliation, input, transform, spriteAnim, controller, animInfo] : viewAnim.each())
    {
        ...
        for (int i = 0; i < spriteAnim.eventCount; i++)
        {
            ...
            {
                OnAnimFrameChanged(entity, animEvent, transform, affiliation, controller, animInfo, input);
            }
        }
    }
}
    ...
    const PlayerControllerInput& input)
{
    ...
    if (animType == AnimType::ATTACK_1)
    {
        ...
        if (animEvent.index == 2)
        {
            ...
            const b2Vec2 center = transform.position + b2Vec2{ s * 1.30f, 1.10f };
            const float radius = 2.5f;
            bool hit = DamageUtils::AttackBox(m_scene, entity, affiliation, damage, filter, center, 0.7f, 0.1f);
    ...
    else if (animType == AnimType::ATTACK_2)
    {
        ...
        else if (animEvent.index == 3)
        {
            const b2Vec2 vertices[6] = {
                position + b2Vec2(s * -0.6f, 2.f),
                position + b2Vec2(s * 0.4f, 1.9f),
                position + b2Vec2(s * 0.8f, 1.5f),
                position + b2Vec2(s * 0.9f, 1.f),
                position + b2Vec2(s * 0.6f, 0.5f),
                position + b2Vec2(s * 0.1f, 0.2f),
            };  
        ...
        else if (animEvent.index == 10)
        {
            ...
            const b2Vec2 boxCenter = transform.position + b2Vec2{ s * 2.f, 1.1f };
            bool hit = DamageUtils::AttackBox(m_scene, entity, affiliation, damage, filter, boxCenter, 1.2f, 0.2f);
            PlayerUtils::PlaySFXHit(m_scene, playerID, SFX_SWORD_HIT_A1, SFXIntensity::STRONG, hit);
        }
        else if (animEvent.index == 11)
        {
            ...
            const b2Vec2 Center = transform.position + b2Vec2{ s * 2.6f, 1.1f };
            bool hit = DamageUtils::AttackCircle(m_scene, entity, affiliation, damage, filter, Center, 0.6f);
            PlayerUtils::PlaySFXHit(m_scene, playerID, SFX_SWORD_HIT_A1, SFXIntensity::STRONG, hit);
        }
    }
    else if (animType == AnimType::ATTACK_3)
    {
        ...
        else if (animEvent.index == 5)
        {
            ...
            const b2Vec2 vertices[3] = {
                position + b2Vec2(s * 0.3f, 0.2f),
                position + b2Vec2(s * 2.2f, 2.1f),
                position + b2Vec2(s * 3.0f, 0.1f),
            };
            bool hit = DamageUtils::AttackPolygon(m_scene, entity, affiliation, damage, filter, vertices, 3);
            PlayerUtils::PlaySFXHit(m_scene, playerID, SFX_SWORD_HIT_B1, SFXIntensity::WEAK, hit);
        }
    }
    else if (animType == AnimType::ATTACK_AIR)
    {
        ...
        if (animEvent.index == 3)
        {
            ...
            const b2Vec2 center = transform.position + b2Vec2{ s * 1.80f, 1.50f };
            const float radius = 2.5f;
            bool hit = DamageUtils::AttackBox(m_scene, entity, affiliation, damage, filter, center, 1.f, 0.1f);
            PlayerUtils::PlaySFXHit(m_scene, playerID, SFX_SWORD_HIT_A1, SFXIntensity::NORMAL, hit);
        }
    }
