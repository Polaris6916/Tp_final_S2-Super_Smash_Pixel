...
UIBaseList::UIBaseList(Scene *scene, int itemCount, UISelectableGroup *group)
    ...
    , m_itemIdx(0)
    , m_isCycle(true)
    , m_listNavigation(Navigation::ALL)
    , m_autoCancel(true)
    , m_soundItem(-1)
{
    SetName("UIBaseList");
...
void UIBaseList::DrawImGui()
{
    ...
    ImGui::PushID(this);
    if (ImGui::CollapsingHeader("UIBaseList"))
    {
        if (ImGui::BeginTable("UIBaseList Members", 2, ImGuiTableFlags_RowBg))
        {
            ImGuiUtils::Member("Item index", m_itemIdx);
            ImGuiUtils::Member("Item count", m_itemCount);
            ImGuiUtils::Member("Is cycle", m_isCycle);
            ImGuiUtils::Member("List navigation", ENUM_STRING(m_listNavigation));
