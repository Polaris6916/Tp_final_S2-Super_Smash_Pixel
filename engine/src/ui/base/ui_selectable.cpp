...
UISelectable::UISelectable(Scene *scene, UISelectableGroup *group)
    ...
    , m_clickedOnRelease(true)
    , m_handlesInputs(false)
    , m_soundClick(-1)
    , m_soundPressed(-1)
    , m_soundSelected(-1)
{
...
UISelectable *UISelectable::GetOnUp()
{
    ...
    while (next != nullptr && next->IsEnabled() == false)
    {
        next = next->m_nextUp;
    }
    return next;
...
void UISelectable::DrawGizmos()
{
    SDL_FRect navRect = GetCanvasNavigationRect();
    SDL_SetRenderDrawColor(g_renderer, 255, 0, 0, 128);
    SDL_RenderFillRect(g_renderer, &navRect);

    SDL_FRect rect = GetCanvasRect();
    SDL_SetRenderDrawColor(g_renderer, 255, 255, 0, 128);
    ...
    if (m_group != nullptr && m_group->GetCursor() != nullptr)
    {
        ...
        SDL_RenderFillRect(g_renderer, &cursorRect);
    }
}

void UISelectable::DrawImGui()
{
    UIObject::DrawImGui();
    std::shared_ptr<ImGuiManagerBase> managerPtr = m_scene->GetImGuiManager();
    ImGuiManagerBase *imGuiManager = managerPtr.get();
    ImGui::PushID(this);
    if (ImGui::CollapsingHeader("UISelectable"))
    {
        if (ImGui::BeginTable("UISelectable Members", 2, ImGuiTableFlags_RowBg))
        {
            ImGuiUtils::Member("State", ENUM_STRING(m_state));
            ImGuiUtils::Member("Clicked on release", m_clickedOnRelease);
            ImGuiUtils::Member("Handles inputs", m_handlesInputs);
            ImGuiUtils::Member("Selectable group", m_group, imGuiManager);
            ImGuiUtils::Member("Next up", m_nextUp, imGuiManager);
            ImGuiUtils::Member("Next down", m_nextDown, imGuiManager);
            ImGuiUtils::Member("Next left", m_nextLeft, imGuiManager);
            ImGuiUtils::Member("Next right", m_nextRight, imGuiManager);
...
{
    if (IsEnabled() == false || makeSound == false) return;
