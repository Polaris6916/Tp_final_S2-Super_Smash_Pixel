...
{
    if (m_selected != nullptr && m_selected->IsEnabled()) return;
...
void UISelectableGroup::DrawImGui()
{
    UIObject::DrawImGui();
    ...
    ImGui::PushID(this);
    if (ImGui::CollapsingHeader("UISelectableGroup"))
    {
        if (ImGui::BeginTable("UISelectableGroup Members", 2, ImGuiTableFlags_RowBg))
        {
            ImGuiUtils::Member("Navigation enabled", m_navigationEnabled);
            ImGuiUtils::Member("Is interactable", m_interactable);
            ImGuiUtils::Member("Cursor", m_cursor, imGuiManager);
            ImGuiUtils::Member("Selected", m_selected, imGuiManager);

            if (ImGuiUtils::BeginSection("Selectables"))
            {
                for (auto &selectable : m_selectableSet)
                {
                    ImGuiUtils::Member("Selectable", selectable, imGuiManager);
                }
