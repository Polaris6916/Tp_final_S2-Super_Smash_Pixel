...
void UIGridLayout::DrawImGui()
{
    ...
    ImGui::PushID(this);
    if (ImGui::CollapsingHeader("UIGridLayout"))
    {
        if (ImGui::BeginTable("UIGridLayout Members", 2, ImGuiTableFlags_RowBg))
        {
            ImGuiUtils::Member("Anchor", ENUM_STRING(m_anchor));
            ImGuiUtils::Member("Row count", m_rowCount);
            ImGuiUtils::Member("Column count", m_colCount);
